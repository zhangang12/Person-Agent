...
const TOOLS = [
  ...
  { name: 'get_evidence', description: '按 ref 拉证据全文(形如 "bundleId/name",可带 ref# 前缀),如 dom/req-body/resp-body/err-stack', inputSchema: { type: 'object', properties: { ref: { type: 'string' }, head: { type: 'number', description: '只取前 N 字符,省略=全文' } }, required: ['ref'] } },
  { name: 'get_dom_subtree', description: '按 CSS 选择器从复现包的完整 DOM 抽某个子树的 outerHTML(优先用它而非整页 dom)', inputSchema: { type: 'object', properties: { bundleId: { type: 'string' }, selector: { type: 'string' } }, required: ['bundleId', 'selector'] } },
  { name: 'get_event_window', description: '从录制时间线取某一步前后 ±N 步的窗口,看用户在出问题前后做了什么。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string' }, step: { type: 'number' }, radius: { type: 'number' } }, required: ['bundleId', 'step'] } },
  { name: 'repro_assert', description: '改完代码后**必须调用**:声明修复"应让什么消失/出现",验证回放时逐条检查。kind: no_console(无匹配报错) / no_element(元素消失) / has_element(元素出现) / no_net(无该 URL 4xx/5xx)。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string', description: '复现包 id(证据包顶部 === 复现包 b_xxx ===)' }, kind: { type: 'string', enum: ['no_console', 'no_element', 'has_element', 'no_net'] }, value: { type: 'string', description: 'no_console: 应不再出现的报错子串;no_element/has_element: CSS 选择器;no_net: URL 子串' }, why: { type: 'string', description: '一句话:断言对应的修复意图' } }, required: ['bundleId', 'kind', 'value'] } },
  ...
  { name: 'scan_impact', description: '**改代码前必须先调用** — 查符号在项目里的所有引用,评估"改了它影响哪些地方"。改了某文件却没扫过对应符号,验证报告标 SUSPICIOUS。底层用 git grep,必须传项目根目录 cwd。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string', description: '复现包 id' }, symbol: { type: 'string', description: '要查引用的符号(函数名/常量名/类名等)' }, cwd: { type: 'string', description: '项目根目录绝对路径' } }, required: ['bundleId', 'symbol', 'cwd'] } },
  { name: 'repro_self_review', description: '**改完代码后必须调用** — self-review 本次修复:summary(改了什么)/risk 1-5(信心)/edge_cases(可空)。缺 review 或信心低,验证报告标 SUSPICIOUS。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string' }, summary: { type: 'string', description: '1-3 句:改了哪些文件、为什么这样改' }, risk: { type: 'number', description: '对修复正确性的自评信心 1-5' }, edge_cases: { type: 'string', description: '可空:没覆盖到的边界场景' } }, required: ['bundleId', 'summary', 'risk'] } },
  { name: 'repro_rollback', description: '验证 FAIL 后把本次改动回滚到 HEAD;files 非空则只回滚那些。**慎用**:会丢失未提交的改动;确认修复方向错误时用它干净重来。', inputSchema: { type: 'object', properties: { cwd: { type: 'string', description: '仓库根目录绝对路径' }, files: { type: 'array', items: { type: 'string' }, description: '可选:仅回滚这些文件;省略=全部改动' }, dryRun: { type: 'boolean', description: 'true 只列出会被回滚的文件,不动手(建议先 dryRun)' } }, required: ['cwd'] } },
  { name: 'bundle_note', description: '多 agent 共享便签:把假设标 confirmed/excluded/maybe + 证据,后续 excluded 不重查、confirmed 不质疑。同 key 覆盖。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string' }, key: { type: 'string', description: '简短假设 key,如 is_cors' }, status: { type: 'string', enum: ['confirmed', 'excluded', 'maybe'] }, evidence: { type: 'string', description: '1-2 句证据' } }, required: ['bundleId', 'key', 'status', 'evidence'] } },
  { name: 'read_notes', description: '读某个复现包当前所有便签(动手前先看,避免重复工作)。', inputSchema: { type: 'object', properties: { bundleId: { type: 'string' } }, required: ['bundleId'] } },
  { name: 'skill_pending_resolves', description: '列出技能回放当前挂起的"运行时解析请求"(回放暂停在某步,等一个运行时才知道的值:物料数据/文件路径/验证码等),可主动调它查漏。', inputSchema: { type: 'object', properties: {} } },
  { name: 'skill_resolve', description: '答复一个挂起的解析请求:把解出的值写回,回放立刻续跑并填入该步字段。**只在确有把握时调用**;只有人知道的值(如短信验证码)不要猜,回复用户说明即可。', inputSchema: { type: 'object', properties: { gateId: { type: 'string', description: '解析请求 id(来自暂停通知或 skill_pending_resolves)' }, value: { type: 'string', description: '解出的值(将按输入步写进目标字段)' }, note: { type: 'string', description: '可选:一句话依据' } }, required: ['gateId', 'value'] } },
  { name: 'skill_relocate', description: '自愈重定位:回放某步元素找不到(页面改版/动态 id)时,按主程序给的意图+当前页元素清单重定位。selector 必须是合法原生 CSS 或 __text__:tag|文本(按可见文本);严禁 :has-text()/:contains()/xpath。命中则回放续跑并回写技能。', inputSchema: { type: 'object', properties: { gateId: { type: 'string', description: '自愈请求 id(来自自愈通知)' }, selector: { type: 'string', description: '合法 CSS 选择器,或 __text__:tag|文本' }, note: { type: 'string', description: '可选:一句话依据' } }, required: ['gateId', 'selector'] } },
  { name: 'skill_refine', description: '提交技能精修结果(主程序请你整理技能时调用):改进后的字段写回,校验后自动落盘并更新技能文档。只传有把握改善的字段。', inputSchema: { type: 'object', properties: { recId: { type: 'string', description: '技能 id(形如 rec_xxx,来自整理请求)' }, title: { type: 'string', description: '≤20字技能名(现有名够好就省略)' }, description: { type: 'string', description: '何时使用:一两句' }, intents: { type: 'object', description: '{事件下标: 这步的人话名},只写能明显改善的步' }, params: { type: 'array', items: { type: 'object' }, description: '[{stepIndex, label}] 提名"每次运行都不同"的输入步为参数' }, success: { type: 'object', description: '{kind:"text"|"css", value} 可自动检查的成功标志' }, notes: { type: 'string', description: '决策点/隐藏偏好/注意事项' } }, required: ['recId'] } },
]
