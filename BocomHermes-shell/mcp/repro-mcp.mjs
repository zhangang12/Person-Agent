...
const TOOLS = [
  ...
  { name: 'skill_relocate', description: '自愈重定位:回放某步元素找不到(页面改版/动态 id)时,主程序带上"这步意图+当前页可交互元素清单"请你重定位。selector 必须是①合法原生 CSS(querySelector 能跑:#id、input[name="x"]、.a>.b:nth-of-type(2)),或②按可见文本匹配的本系统写法 __text__:tag|文本(如 __text__:button|确定)。严禁 :has-text()/:contains()/xpath。命中则回放续跑并回写技能。', inputSchema: { type: 'object', properties: { gateId: { type: 'string', description: '自愈请求 id(来自自愈通知)' }, selector: { type: 'string', description: '合法 CSS 选择器,或 __text__:tag|文本(按可见文本匹配)' }, note: { type: 'string', description: '可选:一句话依据' } }, required: ['gateId', 'selector'] } },
