...
**待办：** ② HTTP 抓包的 GUI 面板（仅本地 127.0.0.1 的 serve / HTTP，不做 HTTPS MITM，不做 remote）;技能 phase-3(明确不做留待立项):日期日历弹层专项、录制端 dialog 按录制答案回放、技能步骤编辑器、OOPIF 跨域 iframe(现按同源/URL 匹配)。详见 [[project-intranet-mode]]。
