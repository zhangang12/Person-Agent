...
> **【已退役 · 2026-07-20】** 本文描述的写死 5 persona 对抗管线(reqanalysis.js/reqflow/reqconfirm/reqplan 窗口)已拆除,
> 由**动态工作流**接替:主 Agent 现场勘察后现编多角度子 Agent 派发(角色不预置),连续上下文里自己对齐汇总,
> 产出后还能同卡继续出详设/查代码(见 `src/window.js` workflowSystemPrompt;需求书 docx 经 doc-mcp/read_document 读取)。
> 本文保留作设计史参考 —— 对抗式读脏需求的思想不变,变的是"角色写死"为"角色现编"。
