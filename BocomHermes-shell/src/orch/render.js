...
function renderPlan(run, facts, event) {
  ...
  return block([
    ...
    '',
    open.length ? '【已知未决】\n' + bullets(open, 12) + '\n' : '',
    '【预算】还能开 ' + left + ' 个节点(上限 ' + num(b.maxNodes) + ')。',
    // ★输出长度是硬约束,不是建议。真机 2026-08-07:模型给了一份很好的 18 片方案,
    //   每片 goal 写了近千字的探索方法论,总长 16439 字 —— 输出到顶被截断,JSON 没闭合,
    //   壳层一个字都用不上,报出来还是一句不相干的「nodes 是空数组」。
    //   原来这里只写"本次不限批量",等于鼓励它一次全给完。
    '【输出长度 —— 硬约束,超了整份作废】整个 JSON 控制在 6000 字以内。做不到就少给几片:',
    '  · 一次最多给 8 片,其余写进 open 并置 more:"unknown" —— 下一轮你还会被问一次,那时再补,不会丢。',
    '  · 每片 goal 压到 200 字以内,只说清【查什么、产出到哪个文件、边界在哪】。',
    '    探索方法论、怎么读文件、回报格式这些【一个字都不要写】—— 工人卡自带一整套规程,写了是重复,',
    '    而且正是这些重复把输出撑爆的。',
    '  · ★宁可少给几片,也不要给半截 JSON:没闭合的 JSON 壳层一个字都用不上,那一轮完全白跑。',
...
function planSchemaText() {
  return block([
    ...
    '  needGrounding=true 表示"我得先看代码才能拆" ——【只要置这个标志、nodes 留空就行】,',
    '    壳层会替你开一个勘察节点,跑完再来问你怎么拆;想自己指定勘察什么,就把疑点写进 open,',
    '    或者干脆自己给一个 kind:"probe" 节点。',
