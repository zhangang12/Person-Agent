...
function planSchemaText() {
  return block([
    ...
    '    "requireEvidence": true, "requireVerdict": false, "verifyCmd": ""',
    '  }],',
    '  "more": "no|unknown",',
    '  "open": ["还说不清的点"],',
    '  "why": "一句话理由"',
    ...
    '  artifacts=该节点必须落盘的文件(壳层会核对存在且非空);requireEvidence=收官必须有构建/测试执行证据;',
    '  requireVerdict=回报必须带 VERDICT 行;verifyCmd=壳层替你跑的一条命令,退出码即判据(不需要就留空串)。',
  ])
}
