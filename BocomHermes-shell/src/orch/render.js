...
function renderPlan(run, facts, event) {
  ...
  return block([
    ...
    str(event) === 'too-narrow'
      ? '【★这是打回重问 —— 上一版被壳层退回了】\n'
        + '  上一版能【同时开跑】的片数比并发还少,并发位空着。原样再给一遍会被再退一次。\n'
...
function composeNodeBrief(run, node) {
  ...
  return block([
    ...
    '  ④ 发现清单 —— 只要你这一片查出了【需要别人复核的具体结论】,就按下面这个块原样输出(没有就整块不写):',
    '',
    '<发现>',
    'F| 严重度(高/中/低) | 一句话说清是什么问题 | 证据(文件:行号 或 命令 或 接口)',
    '</发现>',
    '',
    '  每行一条,以 F| 开头,三段用 | 分隔。壳层会【逐条派新眼睛去核】——',
    '  所以写具体:"订单金额精度丢失"是一条,"代码质量有待提高"不是(核不动的东西不要写进来)。',
    '  条数服从事实:查到几条写几条,没查到就整块不写,不要为了凑数编。',
  ])
}
