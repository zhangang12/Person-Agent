...
function makeReduceSpec(run, targets) {
  ...
  return {
    ...
    goal: [
      ...
      '',
      '【三条硬约束 —— 壳层会机器检查,不过就打回】',
      '  ① 【只交这一份】(' + out + ')。不许把正文拆成几个分册再在这里放目录 ——',
      '     "一个索引 + 三个分册"是最常见的偷懒形态,壳层会数你落了几个文件,多一个就判不过。',
      '     写长没关系,写散不行。',
      '  ② 【至少引用一半上游】。上面列的每一片都要真读过;引用不到一半,说明有片白跑了。',
      '  ③ 【分量要配得上上游】。上游一共多少字,你交的这份不该只有零头 —— 那说明你只写了个目录。',
    ].join('\n'),
  }
}
...
function makeAuditSpec(run, reduceNode) {
  ...
  return {
    ...
    goal: [
      ...
      '',
      // ★判决【不许压在最后】(真机 2026-08-09):原来这段写的是"【收尾】调一次 report_verdict",
      // 于是机判凭据成了整条最长任务的终点 —— 回合一被中断(实测:被绕圈看门狗误杀在第 9 步,
      // 它刚说完"现在我有完整的证据链"),前面所有工作都白做:verdict 闸判不过 → 整片重做 →
      // 而验证片按规程不许补做,一次重做就烧掉一次 attempt,两次就永久失败。
      // 【最贵的东西不该放在最容易丢的位置】—— 有初步结论就先落一次判决,后面再更新;
      // 工具可以重复调用,最后一次为准。这样任何中断都至少留下一个可判的结论。
      '【判决要【尽早】给,不要留到最后】只要已经形成初步结论,就【立刻】调一次 MCP 工具',
      '  report_verdict(PASS / FAIL)先把它落下来 —— 后面继续核到新东西,再调一次覆盖即可(最后一次为准)。',
      '  ★不要等全部核完才调:一旦这一轮被中断,没落过判决就等于整片白做、还要从零重来。',
