...
function skillMd(rec) {
  ...
  { const pp = rec.postPipeline || rec.postWorkflow; if (pp && pp.goal) L.push('', '## 下载后任务编排', '回放导出/下载文件后,自动开一张【单 Agent 任务编排卡】读文件并按目标办:', '> ' + String(pp.goal).slice(0, 300)) }
  if (rec.skillNotes) L.push('', '## 注意事项(决策点/隐藏偏好)', rec.skillNotes)   // Agent 精修补的"Codex 式"决策点说明
  return L.join('\n')
}

// 「下载后任务编排」目标合成(纯函数):技能回放把文件导出/下载到本地后,把文件绝对路径接进【任务编排卡】的目标文本。
// template=用户填的人话目标(如"把导出的用户反馈表做成分析报告");files=本次回放捕获到的下载文件绝对路径数组。
// 值全是文件路径(业务产物,非机密),可安全进模型;显式要求单 Agent 用读取/表格解析工具打开,别臆测。
function composePostPipelineGoal(skillName, template, files) {
...
module.exports = { RECORDER_JS, selExpr, findElExpr, frameFor, safeOrigin, applyParams, applyBaseUrl, JS_LIKE, diffReport, coverageHits, clusterErrs, compactEvents, humanGateHint, markHumanGates, upgradeToSkill, skillMd, composePostPipelineGoal, applyRefinePatch, rowToParamValues, relocateSelectors, takeoverDigest, redactRec, anchorExpr, t2s }
