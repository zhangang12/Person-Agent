...
module.exports = function initWindow(S, { ipcMain, app, BrowserWindow, WebContentsView, screen, dialog, Tray, Menu, nativeImage, shell, path, fs, oc, log }) {
  ...
  function workflowSystemPrompt(dir, backendDir) {
    return [
      ...
      '8. 【收尾必验证,不靠信念交差】改了代码就跑测试 / 构建 / 驱动一遍看真过;下了关键结论就派子 Agent 交叉核实,别自证。一波不够深、或冒出新待挖点就【再派一波】—— 目标是把事做透,不是一遍浅 pass 交差。交付前自查产出:还没有任何落盘文件 = 没完成,按第 7 条补上 MD 文档再交付。',
  ...
  function orchestratorSystemPrompt(dir, backendDir, orchTag) {
    return [
      ...
      '   M = max(⌈N/20⌉, ⌈总量/150KB⌉),文件就近归组分成 ≥M 片。例:N=34、总量 1MB → M=max(2,7)=7 片起步 —— 拆 4 片 = 每片 250KB,超预算 1.7 倍,属错拆。',
      ...
      '3. 【第一轮只出方案,等批准】用 todowrite 列出:预检结论(N 与总量)、片数 M 的预算演算(max(⌈N/20⌉, ⌈总量/150KB⌉))、每个分片的一句话目标 + 自估大小(文件数/总量,超线即错拆必须重分)、边界(干什么不干什么)、产出落盘路径(docs/<主题>/<分片名>.md)、索引棒打算。然后结束这轮等用户点【开始执行】—— 批准前【不派任何分片】。',
      ...
      '6. 【索引棒收口 = 联合 LLM-wiki】分片全部完成后,调 workflow_result 逐个取回成果(只取一句话结论与路径,别拉全文),再调 run_workflow 派【索引 Agent】(goal 同样以 [orch:' + orchTag + '] 开头):总目标 + 每个分片一句话结论 + 产出路径清单 + 分歧点,要求它产出一份【联合 LLM-wiki 开发文档】' + (dir ? dir + '/docs/' : 'docs/') + '<主题>/README.md —— 【不是只有链接的索引】:总览与业务闭环 + 每分片一章(核心结论/关键规则/数据流/重要 file:行号,这一章本身就够用) + 章末链到分片详文档 + 分歧点仲裁。读者只看 README 就能掌握全局,要细节再按链接下钻。分片全文【严禁】贴进 goal(只给路径与一句话);它读不过来,就让它派子 Agent 每片一个分头读文档、自己综合,还是文档接力。',
