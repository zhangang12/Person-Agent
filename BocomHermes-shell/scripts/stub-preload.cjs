...
contextBridge.exposeInMainWorld('BocomHermes', {
  ...
  runApprove: async () => { flags.runAction = 'approve'; return { ok: true } },
  runReject: async (_id, note) => { flags.runAction = 'reject:' + note; return { ok: true } },
  runNote: async (_id, text) => { flags.runAction = 'note:' + text; return { ok: true } },
  runAbort: async () => { flags.runAction = 'abort'; return { ok: true } },
  runRetryNode: async (_id, nodeId) => { flags.runAction = 'retry:' + nodeId; return { ok: true } },
  runResume: async () => { flags.runAction = 'resume'; return { ok: true } },
