...
section('用例6l:C+D —— 汇总的实质性闸 + 新眼睛验收', () => {
  ...
  // ★判决不许压在最后(真机 2026-08-09):原来写的是"【收尾】调一次 report_verdict",
  // 机判凭据成了最长任务的终点 —— 回合被中断(实测被绕圈看门狗误杀在第 9 步)就整片白做:
  // verdict 闸判不过 → 重做,而验证片不许补做,两次重做直接永久失败。
  // 最贵的东西不该放在最容易丢的位置:有初步结论就先落一次,后面覆盖。
  ok('★★判决要【尽早】给,不许留到收尾(中断一次就白做整片)',
    /尽早/.test(a.goal) && /立刻/.test(a.goal) && !/【收尾】调一次/.test(a.goal), (a.goal.match(/判决要[\s\S]{0,60}/) || [])[0])
  ok('  说清可以重复调、最后一次为准(否则它不敢先落一个临时结论)',
    /再调一次覆盖/.test(a.goal) && /最后一次为准/.test(a.goal))
  ok('  把代价讲明白(不落判决 = 整片白做重来)', /整片白做/.test(a.goal))
