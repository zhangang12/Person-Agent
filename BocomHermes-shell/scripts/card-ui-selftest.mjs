...
function fakeEl(tag) {
  ...
  const el = new Proxy(store, {
    get: (t, k) => {
      ...
      if (k === 'remove') return () => { const p = t.parentNode; if (p && p.children) { const i = p.children.indexOf(el); if (i >= 0) p.children.splice(i, 1) } t.parentNode = null }   // 对齐真 DOM:remove 真从父节点 children 里摘掉,并清 parentNode
...
const documentStub = {
  getElementById: (id) => {
    const hit = created.find((c) => c.el && c.el.id === id && c.el.parentNode)   // 先找动态创建且带 id 的真元素(对齐真 DOM;自动创建只在确实没有时兜底)
...
console.log('用例19:体验包 —— 截断提示 / sticky待批准 / 水位提醒 / DOM回收 / 流式增量渲染')
{
  ...
  await exported.turnFn('z')   // 未超线的下一轮:占位不许闪没(bug 复验)
  const capNote2 = created.filter((c) => c.el.id === 'feedCapNote').pop()
  ok('未超线的下一轮收纳占位仍在', !!capNote2 && /已收纳 \d+ 条早期消息/.test(capNote2.el.textContent || ''), capNote2 && capNote2.el.textContent)
