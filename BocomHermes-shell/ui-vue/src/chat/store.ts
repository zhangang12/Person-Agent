...
setInterval(() => {
  ...
  if (sil >= 300000 && !hangNag300) {
    ...
    // 措辞跟着主进程的判死线走(session.js 挂死看门狗,2026-08-04 由 300s 调到 1800s:
    // 内网慢端点 prefill + 长 reasoning 常超 5 分钟没有 part 变化,5min 把"慢"误杀成"挂死")。
    // ★这里只报"已经静默了多久",不替它下"挂死了"的结论,也不承诺马上会被中断 ——
    //   5 分钟在新口径下还远没到判死线,写成"壳层会自动中断"会让用户以为马上有人管,其实还要等 25 分钟。
    addNote(s.shardMode || s.orchMode
      ? '⚠ 已 5 分钟没有新输出 —— 内网慢端点长考也可能这样。无人值守卡不用管:真到 30 分钟仍无动静,壳层看门狗会自动中断本轮并交回主控重派'
      : '⚠ 已 5 分钟没有输出 —— 可能是模型在长考(内网慢端点常见),也可能是网关挂死。想不等了就按 Esc 中断,再点「重试本轮」')
  }
