...
try {
  ...
  ok(init.result?.serverInfo?.name === 'tianshu-browser', 'initialize 返回 serverInfo')
  ...
  const html = 'data:text/html,' + encodeURIComponent('<title>TS-OK</title><body><h1 id=h>HELLO_TIANSHU</h1>')
  ...
  if (nav.result?.isError) {
    ...
    ok(/HELLO_TIANSHU/.test(txt.result?.content?.[0]?.text || ''), 'get_text 取到正文')
